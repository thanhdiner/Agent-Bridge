...
class Program
{
    static void Main()
    {
        ...
        string pngPath = Path.Combine("src", "AgentBridge.Desktop", "Assets", "agentbridge-icon-512.png");
        bmp512.Save(pngPath, ImageFormat.Png);

        // Save valid ICO containing PNG data
        using var msPng = new MemoryStream();
        bmp.Save(msPng, ImageFormat.Png);
        byte[] pngBytes = msPng.ToArray();

        string icoPath = Path.Combine("src", "AgentBridge.Desktop", "Assets", "agentbridge.ico");
        ...
        Console.WriteLine("Valid WPF ICO and PNG generated successfully!");
    }
}
