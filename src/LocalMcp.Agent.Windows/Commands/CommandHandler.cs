...
public sealed partial class CommandHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        else if (command is GitPushCommand gitPushCommand)
        {
            return await HandleGitPushAsync(gitPushCommand, cancellationToken);
        }
        ...
        else if (command is AppOpenCommand appOpenCommand)
        {
        ...
        else if (command is ScreenClickCommand screenClickCommand)
        {
            return WindowActionToJson(await LocalMcp.Agent.Windows.UiAutomation.ScreenInputDispatch.HandleClickAsync(_uiAutomationExecutor, screenClickCommand, cancellationToken));
        }
        else if (command is ScreenDragCommand screenDragCommand)
        {
            return WindowActionToJson(await LocalMcp.Agent.Windows.UiAutomation.ScreenInputDispatch.HandleDragAsync(_uiAutomationExecutor, screenDragCommand, cancellationToken));
        }
        else if (command is ScreenScrollCommand screenScrollCommand)
        {
            return WindowActionToJson(await LocalMcp.Agent.Windows.UiAutomation.ScreenInputDispatch.HandleScrollAsync(_uiAutomationExecutor, screenScrollCommand, cancellationToken));
        }
        ...
        else if (command is UiExpandCollapseCommand uiExpandCollapseCommand)
        {
            return WindowActionToJson(await LocalMcp.Agent.Windows.UiAutomation.UiExpandCollapseDispatch.HandleAsync(_uiAutomationExecutor, uiExpandCollapseCommand, cancellationToken));
        }
        else if (command is UiToggleCommand uiToggleCommand)
        {
            return WindowActionToJson(await LocalMcp.Agent.Windows.UiAutomation.UiToggleDispatch.HandleAsync(_uiAutomationExecutor, uiToggleCommand, cancellationToken));
        }
        else if (command is UiRangeValueCommand uiRangeValueCommand)
        {
        ...
        else if (command is UiGridSelectCommand uiGridSelectCommand)
        {
            return WindowActionToJson(await LocalMcp.Agent.Windows.UiAutomation.UiGridSelectDispatch.HandleAsync(_uiAutomationExecutor, uiGridSelectCommand, cancellationToken));
        }
        ...
        else if (command is UiFindCommand uiFindCommand)
        {
            return await HandleUiFindAsync(uiFindCommand, cancellationToken);
        }
    ...
    private async Task<CommandResult<JsonElement>> HandleGitPushAsync(GitPushCommand command, CancellationToken cancellationToken)
    {
        var error = _pathPolicy.AuthorizeReadDirectory(command.Path, out var normalizedPath);
        if (error is not null)
        {
        ...
        var result = await _fileSystemExecutor.GitPublishAsync(
            normalizedPath,
            command.Remote,
            command.Branch,
            command.SetUpstream,
