...
[McpServerToolType]
public sealed class UiFocusTools
{
    private readonly ICommandDispatcher _dispatcher;
    private readonly IAuthorizationService _authorizationService;
    private readonly IHttpContextAccessor? _httpContextAccessor;
    private readonly ILogger<UiFocusTools> _logger;

    public UiFocusTools(
        ICommandDispatcher dispatcher,
        IAuthorizationService authorizationService,
        ILogger<UiFocusTools> logger,
    ...
    [McpServerTool(
        ...
        Idempotent = true,
        OpenWorld = false),
     Description("Focuses one exact Windows UI Automation control, restoring and foregrounding its window, scrolling the control into view when supported, and verifying keyboard focus. Requires dev:execute scope.")]
    public async Task<CallToolResult> FocusAsync(
        ...
        [Description("Optional exact control type such as Edit, Document, Button, ListItem, or TreeItem")] string? controlType = null,
        [Description("Zero-based index when multiple controls match (default: 0, hard limit: 1000)")] int occurrenceIndex = 0)
    {
        ...
        var command = new UiFocusCommand
        {
        ...
        try
        {
            var result = await _dispatcher.SendAsync<UiFocusResult>(command, RequestToken());
        ...
        {
            _logger.LogError(ex, "Unexpected ui_focus failure for device {DeviceId}", deviceId);
