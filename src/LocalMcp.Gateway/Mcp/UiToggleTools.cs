...
[McpServerToolType]
public sealed class UiToggleTools
{
    private readonly ICommandDispatcher _dispatcher;
    private readonly IAuthorizationService _authorizationService;
    private readonly IHttpContextAccessor? _httpContextAccessor;
    private readonly ILogger<UiToggleTools> _logger;
    public UiToggleTools(
        ICommandDispatcher dispatcher,
        IAuthorizationService authorizationService,
        ILogger<UiToggleTools> logger,
    ...
    public async Task<CallToolResult> ToggleAsync(
        ...
        [Description("Optional exact control type such as CheckBox, Button, or MenuItem")] string? controlType = null,
        ...
        [Description("Whether to focus the target window before changing state (default: true)")] bool focusWindow = true)
    {
        ...
        if (!UiToggleActions.TryNormalize(action, out var normalizedAction))
            return Error("INVALID_REQUEST", "action must be one of: on, off, toggle.");
        ...
        try
        {
            var result = await _dispatcher.SendAsync<UiToggleResult>(command, RequestToken());
        ...
        {
            _logger.LogError(ex, "Unexpected ui_toggle failure for device {DeviceId}", deviceId);
