...
[McpServerToolType]
public sealed class UiStateTools
{
    private readonly ICommandDispatcher _dispatcher;
    private readonly IAuthorizationService _authorizationService;
    private readonly IHttpContextAccessor? _httpContextAccessor;
    private readonly ILogger<UiStateTools> _logger;

    public UiStateTools(
        ICommandDispatcher dispatcher,
        IAuthorizationService authorizationService,
        ILogger<UiStateTools> logger,
    ...
    [McpServerTool(
        ...
        Idempotent = true,
        OpenWorld = false),
     Description("Reads enabled, focused, offscreen, selected, checked, and expanded state from one Windows UI Automation control. Pattern-specific states are null when unsupported. Requires dev:execute scope.")]
    public async Task<CallToolResult> GetStateAsync(
        ...
        [Description("Optional exact control type such as CheckBox, MenuItem, TreeItem, TabItem, or Button")] string? controlType = null,
        ...
        [Description("Whether to focus the target window before reading (default: false)")] bool focusWindow = false)
    {
        ...
        var command = new UiGetStateCommand
        {
        ...
        try
        {
            var result = await _dispatcher.SendAsync<UiGetStateResult>(command, RequestToken());
        ...
        {
            _logger.LogError(ex, "Unexpected ui_get_state failure for device {DeviceId}", deviceId);
